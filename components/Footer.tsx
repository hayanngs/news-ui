// components/Footer.tsx
import Link from "next/link"
import { SocialIconsFooter } from "@/components/SocialIcons"

export function Footer() {
  return (
    <footer style={{ fontFamily: "var(--fonte-ui)", marginTop: 40 }}>
      <div style={{ background: "var(--azul-escuro)", padding: "28px 0" }}>
        <div className="w-portal" style={{
          display: "grid", gap: 32,
          gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))"
        }}>

          {/* Logo + redes sociais */}
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              <div style={{ width: 32, height: 32, background: "#fff", borderRadius: 3, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontFamily: "var(--fonte-titulo)", fontWeight: 700, fontSize: 18, color: "var(--azul)" }}>N</span>
              </div>
              <span style={{ fontFamily: "var(--fonte-titulo)", fontWeight: 700, fontSize: 18, color: "#fff" }}>
                Portal Notícias
              </span>
            </div>
            <p style={{ fontSize: 12, color: "rgba(255,255,255,0.55)", lineHeight: 1.6, marginBottom: 4 }}>
              Jornalismo independente.<br />Goiás e Brasil.
            </p>
            {/* Ícones no footer */}
            <SocialIconsFooter />
          </div>

          {/* Navegação */}
          <div>
            <p style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "rgba(255,255,255,0.4)", marginBottom: 12 }}>
              Navegação
            </p>
            <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                { href: "/", label: "Início" },
                { href: "/sobre", label: "Sobre" },
                { href: "/pessoas", label: "Pessoas" },
                { href: "/direitos", label: "Direitos" },
              ].map(l => (
                <li key={l.href}>
                  <Link href={l.href} className="footer-link">{l.label}</Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contato */}
          <div>
            <p style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "rgba(255,255,255,0.4)", marginBottom: 12 }}>
              Contato
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 13, color: "rgba(255,255,255,0.65)" }}>
              <span>redacao@portal.com.br</span>
              <span>contato@portal.com.br</span>
            </div>
          </div>
        </div>
      </div>

      <div style={{ background: "#001f33", padding: "10px 0" }}>
        <div className="w-portal" style={{
          display: "flex", justifyContent: "space-between",
          fontSize: 11, color: "rgba(255,255,255,0.35)",
          flexWrap: "wrap", gap: 4
        }}>
          <span>© {new Date().getFullYear()} Portal Notícias. Todos os direitos reservados.</span>
          <span>Desenvolvido com Next.js + Java</span>
        </div>
      </div>
    </footer>
  )
}
